...
echo -e "${YELLOW}Checking dashboard.local and app.local IP mapping...${NC}"
...
HOSTNAMES="dashboard.local app.local"

# Check if any of these hostnames exist in /etc/hosts
LINE=$(grep -E "\s+(dashboard\.local|app\.local)" /etc/hosts || true)

if [ -z "$LINE" ]; then
  # Neither hostname found, add both on a new line
  echo -e "${YELLOW}Adding new entry for dashboard.local and app.local to /etc/hosts${NC}"
  ...
  # Ensure both hostnames are present (avoid duplicates)
  ...
  # Replace the line for either hostname with new IP and updated hosts
...
grep -E "\s+(dashboard\.local|app\.local)" /etc/hosts || echo "No entries found."
